...
export class MeshEarth extends TEvent {
    constructor(tscene: SLTScene, resource: TResource) {
        ...
        tscene.on('RESIZE', (size: { width: number, height: number }) => {
        ...
        load.event.on('end', () => {
            const texture1 = resources['EarthDaymap'],
                texture2 = resources['EarthNightmap'],
                texture3 = resources['EarthSpecularClouds'];
